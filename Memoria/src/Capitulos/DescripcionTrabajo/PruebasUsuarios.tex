...
Para cada árbol que debería ser creado se hicieron dos escenarios distintos, uno en el que se deba usar la herramienta y otro en el que el NPC ejecuta un árbol creado a mano a través del sistema de Blueprints (sistema de programación visual de Unreal).
...
El segundo escenario consta de un objetivo, dos flores rojas (una de ellas en frente del objetivo para que el NPC tenga que pasar por delante de él) y un punto de patrulla alejado del objetivo.

El formulario, que se puede encontrar en el apéndice \ref{appendix:formularioUsuarios}, mide la usabilidad y la calidad de la herramienta frente a BTs creados por un humano.
...
cells   = {font=\footnotesize\linespread{0.84}\selectfont},
    ...
    U3  & 100   \\
    Media  & 90.83   \\
    Desviación estándar  & 15.88   \\ \hline
...
\begin{longtblr}[
    caption = {Opinión de los usuarios sobre los tiempos de ejecución de la herramienta generadora de árboles de comportamiento del 1 al 5, siendo 1 una opinión desfavorable y 5 la más favorable.},
    label = {tab:OpTemp}
]{
colspec={|l|l|},
rowhead=1,
...
cells   = {font=\footnotesize\linespread{0.84}\selectfont},
    ...
    U3  & 4   \\
    Media  & 4.33   \\
    Desviación estándar  & 0.58   \\ \hline

\end{longtblr}

\subsection{Resultados de la calidad y disminución de carga de trabajo}

Siguiendo con la calidad de los árboles de comportamiento, podemos observar en las tablas \ref{tab:Calidad} y \ref{tab:Ejecucion} que la calidad de los árboles es inferior a los generados por un humano (con una media en las respustas del 80\% y una desviación típica de 1), pero que cumplen con la ejecución esperada (con una media de las respuestas del 80\% y una desviación típica de 0).
...
\begin{longtblr}[
    caption = {Respuestas de los usuarios sobre la calidad de los árboles de comportamiento generados por la herramienta, donde 1 es que no están de acuerdo con la afirmación de que la calidad de los árboles generados por la herramienta es inferior y 5 es que están completamente de acuerdo.},
    label = {tab:Calidad}
]{
colspec={|l|l|},
rowhead=1,
...
cells   = {font=\footnotesize\linespread{0.84}\selectfont},
    ...
    U3  & 4   \\
    Media  & 4.00   \\
    Desviación estándar  & 1.00   \\ \hline

\end{longtblr}
\begin{longtblr}[
    caption = {Respuestas de los usuarios sobre la ejecución esperada de los árboles de comportamiento, donde 1 es que consideran una mala ejecución y 5 significa una ejecución correcta.},
    label = {tab:Ejecucion}
]{
colspec={|l|l|},
rowhead=1,
...
cells   = {font=\footnotesize\linespread{0.84}\selectfont},
    ...
    U3  & 4   \\
    Media  & 4.00   \\
    Desviación estándar  & 0.00   \\ \hline
...
%     \caption[Opinión de la calidad de los árboles de comportamiento]{Respuestas de los usuarios sobre la calidad de los árboles de comportamiento generados por la herramienta, donde 1 es que no están de acuerdo con la afirmación de que la calidad de los árboles generados por la herramienta es inferior y 5 es que están completamente de acuerdo.}
%     \label{fig:Calidad}
% \end{figure}
...
\begin{longtblr}[
    caption = {Respuestas de los usuarios sobre la facilidad para modificar los árboles de comportamiento generados, donde 1 significa que es dificil y 5 significa que es fácil.},
    label = {tab:Mod}
]{
colspec={|l|l|},
rowhead=1,
...
cells   = {font=\footnotesize\linespread{0.84}\selectfont},
    ...
    U3  & 5   \\
    Media  & 4.33   \\
    Desviación estándar  & 1.15   \\ \hline

\end{longtblr}

\begin{longtblr}[
    caption = {Respuestas de los usuarios sobre su opinión de si la herramienta puede disminuir (5) o no (1) la carga de trabajo de los diseñadores.},
    label = {tab:Carga}
]{
colspec={|l|l|},
rowhead=1,
...
%     \caption[Opinión de los usuarios sobre si los resultados del árbol de comportamiento generado son fácilmente modificables]{Respuestas de los usuarios sobre la facilidad para modificar los árboles de comportamiento generados, donde 1 significa que es dificil y 5 significa que es fácil.}
%     \label{fig:Mod}
% \end{figure}
