...
Para poder desarrollar la herramienta se han creado unos entornos de pruebas basados en Unreal Engine 5.7.
...
Una vez se ha especificado todo lo necesario para la generación de los \glspl{BT} se ha procedido a la conexión con la herramienta que los genera.
Como se ha mencionado en la sección \ref{sec:ModeloBT}, por comodidad del usuario es el propio entorno quien lanza el proceso de Python.
Para ello se llama a la función ``ExecProcess'' de la clase de Unreal ``PlatformProcess'' con ``python'' como el proceso que debe ser lanzado y como argumentos la ruta del script de la herramienta, las listas de tareas (la lista de tareas que requieren de una variable de la \gls{BB} y la lista de las que no), la lista de decoradores, el comportamiento buscado y la lista de entradas de la \gls{BB}. 